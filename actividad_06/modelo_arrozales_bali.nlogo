...
to setup
  ...
  set radio_plaga 2
  set radio_cosecha 1
  set prob_seleccion_aleatoria .05
  set max_bloque 4
...
to go
  ...
  ask patches [ elegir_siguiente_itinierario]
...
to calcular_cosecha
  ;; Fraccion de vecinos con el mismo itinerario
  let f_p count vecinos_plaga with [ I_t = [I_t] of myself ] / count vecinos_plaga

  ;; Fraccion de parches con el mismo itinerario
...
to elegir_siguiente_itinierario
  (ifelse
    regla_decision = "maximo" [
    ...
    regla_decision = "aleatorio" [
      set I_t1 one-of itinerarios
    ]

    regla_decision = "mayoria"[
      let posibles modes ([I_t] of vecinos_cosecha)
      set I_t1 one-of posibles
    ]

    regla_decision = "minoria"[
      ;; Bien rústico y problablemente ineficiente, pero me desesperé con la sintaxis :(

      let cant_0 count vecinos_cosecha with [ I_t = 0 ]
      let cant_1 count vecinos_cosecha with [ I_t = 1 ]
      let cant_2 count vecinos_cosecha with [ I_t = 2 ]
      let cant_3 count vecinos_cosecha with [ I_t = 3 ]

      let it-cant (list (list 0 cant_0) (list 1 cant_1) (list 2 cant_2) (list 3 cant_3))
      let cantidad_min min (list cant_0 cant_1 cant_2 cant_3 )

      let posibles []

      foreach it-cant [ x -> if item 1 x = cantidad_min [set posibles lput (item 0 x) posibles ]    ]
